...
if __name__ == '__main__':
    ...
    for i in range(len(file_list)):
        ...
        output_filename = output_dir + "/" + "S_" + args.model.split('/')[-1] + ".txt"
        ...
        a = pd.DataFrame(list(zip(article_ids, drugs_list, symptoms)), columns=['article_id', 'symptoms', 'drugs'])
        a.to_excel(args.output + '/extracted_data_S_' + args.model.split('/')[-1] + '.xlsx', index=False)
